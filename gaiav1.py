...
def process_rosbag_with_localization(bag_path, output_folder="bev_localisation_output"):
    ...
    try:
        ...
        while True:
            ...
            gps_x = gps_row["utm_easting_noisy"].values[0] - center[0]
            gps_y = gps_row["utm_northing_noisy"].values[0] - center[1]
            particles[:, 0] = gps_x + np.random.normal(0, PARTICLE_STD, PARTICLE_COUNT)
            particles[:, 1] = gps_y + np.random.normal(0, PARTICLE_STD, PARTICLE_COUNT)
            ...
            bev_trunks = np.zeros_like(bev)
            bev_poles = np.zeros_like(bev)
            ...
            if results.masks is not None:
                ...
                for i, mask in enumerate(masks):
                    ...
                    # Overlay semantic color on RGB image
                    color_mask = (np.array([0, 255, 0]) if class_id == 2 else np.array([0, 0, 255]))  # Green for pole, red for trunk
                    rgb_vis[binary_mask.astype(bool)] = (
                        0.5 * rgb_vis[binary_mask.astype(bool)] + 0.5 * color_mask
                    ).astype(np.uint8)

                    # Project to BEV
                    if class_id == 4:
                        bev_trunks = overlay_mask_on_bev(binary_mask, depth, bev_trunks, fx, fy, cx, cy, color=(0, 0, 255))
                    elif class_id == 2:
                        bev_poles = overlay_mask_on_bev(binary_mask, depth, bev_poles, fx, fy, cx, cy, color=(0, 255, 0))

            weights = measurement_likelihood(map_bev_poles, map_bev_trunks, bev_poles, bev_trunks, particles)
            ...
            visualize_particles(all_coords, particles, frame_idx, output_folder, trajectory, rgb_vis)
            frame_idx += 1
            pbar.update(1)
    except RuntimeError:
